...
import { ArduinoSettings, SerialPort } from '../libs/clients/race-device/race-device.models';
...
export class RaceDeviceSettingsComponent {
  ...
  protected onTest(): void {
    ...
    this.client.testConnection({ mode: this.mode, endpoint: this.endpoint, arduino: this.arduino }).subscribe({
      next: (result) => {
