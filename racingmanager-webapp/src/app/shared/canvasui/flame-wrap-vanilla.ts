/*
 * FlameWrap — vendored from Canvas UI (https://canvasui.dev).
 * Upstream: components/canvasui/FlameWrapVanilla.ts
 *   npx shadcn@latest add @canvas-ui/flame-wrap-vanilla
 *
 * Copied in rather than installed: Canvas UI ships components as source, and this
 * file has zero dependencies. Local changes are limited to what this repo's
 * tsconfig/eslint require (bracket access on the uniform record, non-empty catch)
 * so it stays diffable against upstream.
 *
 * The full effect (content shimmer, char, melt-over-text) needs the HTML-in-Canvas
 * API, which is Chrome-origin-trial only. Without it the code degrades to drawing
 * the fire around the element rect -- see supportsHtmlInCanvas().
 */
...
export function createFlameWrap(
  ...
  // Defers to start(), which is a hoisted declaration further down this scope.
  const wake = () => start();
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // The element capture can fail mid-frame; the next paint retries.
      }
    };
  }
