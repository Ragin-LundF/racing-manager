...
class ParticipantRepository {
    ...
    fun findByEventIdAndStartNumber(eventId: UUID, startNumber: Int): ParticipantEntity? {
        return transaction {
    ...
    fun insert(participant: ParticipantEntity): InsertStatement<Number>? {
        return transaction {
    ...
    fun update(participant: ParticipantEntity): Int {
        return transaction {
            ParticipantTable.update(where = { ParticipantTable.id eq participant.id }) {
                it[startNumber] = participant.startNumber
                it[firstName] = participant.firstName
                it[lastName] = participant.lastName
                it[club] = participant.club
    ...
    fun updateSortOrders(updates: List<Pair<UUID, Int>>) {
        transaction {
            updates.forEach { (id, order) ->
                ParticipantTable.update(where = { ParticipantTable.id eq id }) {
    ...
    fun deleteByEventId(eventId: UUID): Int {
        return transaction {
    ...
    fun maxStartNumberByEventId(eventId: UUID): Int? {
        return transaction {
            ParticipantTable.selectAll()
                .where { ParticipantTable.eventId eq eventId }
    ...
    // Event seed
    fun findSeedByEventId(eventId: UUID): EventSeedEntity? {
        return transaction {
    ...
    fun upsertSeed(seed: EventSeedEntity) {
        return transaction {
            val existing = EventSeedTable.selectAll().where { EventSeedTable.eventId eq seed.eventId }.singleOrNull()
            if (existing != null) {
                EventSeedTable.update(where = { EventSeedTable.eventId eq seed.eventId }) {
