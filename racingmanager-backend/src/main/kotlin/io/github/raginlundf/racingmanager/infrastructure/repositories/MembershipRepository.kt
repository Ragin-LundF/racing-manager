...
class MembershipRepository {
    ...
    fun findByUserId(userId: UUID): List<MembershipEntity> = transaction {
        MembershipTable.selectAll().where { MembershipTable.userId eq userId }
            .map { it.toMembershipEntity() }
    }

    fun findByUserAndTenant(userId: UUID, tenantId: UUID): MembershipEntity? = transaction {
    ...
    fun findByTenantId(tenantId: UUID): List<MembershipEntity> = transaction {
        MembershipTable.selectAll().where { MembershipTable.tenantId eq tenantId }
            .map { it.toMembershipEntity() }
    }

    fun updateRoleAndStatus(userId: UUID, tenantId: UUID, role: UserRole, status: MembershipStatus, updatedAt: kotlin.time.Instant) = transaction {
        MembershipTable.update({ (MembershipTable.userId eq userId) and (MembershipTable.tenantId eq tenantId) }) {
    ...
    fun insert(membership: MembershipEntity) = transaction {
