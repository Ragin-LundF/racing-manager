...
fun Route.syncRoutes(jwtService: JwtService, syncService: SyncService, deploymentMode: DeploymentMode) {
    post("/api/v1/tenant/local-instances/pairing-token") {
        ...
        when (val result = syncService.issuePairingToken(principal.tenantId)) {
    ...
    post("/api/v1/local-instances/pair") {
        ...
        when (val result = syncService.pair(pairingCode, localInstanceId)) {
            is PairResult.Success -> call.respond(
                status = HttpStatusCode.Created,
                message = PairResponseModel(localInstanceId = result.instance.id.toString(), tenantId = result.instance.tenantId.toString()),
            )
    ...
    get("/api/v1/tenant/local-instances") {
        ...
        call.respond(syncService.listInstances(principal.tenantId).map { it.toResponseModel() })
    }
    ...
    post("/api/v1/tenant/local-instances/{id}/sync-results") {
        ...
        when (val result = syncService.syncResults(principal.tenantId, instanceId, eventId, resultsJson, principal.userId)) {
