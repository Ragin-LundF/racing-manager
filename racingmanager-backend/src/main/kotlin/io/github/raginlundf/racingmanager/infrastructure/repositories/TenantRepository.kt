...
class TenantRepository {
    ...
    fun findById(id: UUID): TenantEntity? = transaction {
        TenantTable.selectAll().where { TenantTable.id eq id }
            .singleOrNull()
            ?.toTenantEntity()
    }

    fun findBySlug(slug: String): TenantEntity? = transaction {
        TenantTable.selectAll().where { TenantTable.slug eq slug }
            .singleOrNull()
            ?.toTenantEntity()
    }

    fun findAll(): List<TenantEntity> = transaction {
        TenantTable.selectAll().map { it.toTenantEntity() }
    }

    fun insert(tenant: TenantEntity) = transaction {
        TenantTable.insert {
            it[id] = tenant.id
            it[slug] = tenant.slug
    ...
    fun update(tenant: TenantEntity) = transaction {
